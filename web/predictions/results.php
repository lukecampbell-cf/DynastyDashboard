...
<?php if ($card === null): ?><div class="alert alert-error">That prediction card was not found.</div><?php else: ?>
...
<div class="results-list"><?php foreach ($card['picks'] as $pick): ?><article class="panel result-row"><div><span class="position-tag"><?php echo predictions_escape($pick['position']); ?></span><h3><?php echo predictions_escape($pick['player_name']); ?></h3><p class="panel-copy"><?php echo predictions_escape($pick['selection']); ?> <?php echo number_format((float) $pick['line_taken'], 1); ?> · Actual <?php echo $pick['actual_points'] === null ? '—' : number_format((float) $pick['actual_points'], 1); ?></p></div><strong class="result-badge result-<?php echo strtolower(predictions_escape($pick['result'])); ?>"><?php echo predictions_escape($pick['result']); ?></strong></article><?php endforeach; ?></div>
