...
$root = sys_get_temp_dir() . '/predictions-phase6-' . bin2hex(random_bytes(5));
...
function phase6_expect(bool $condition, string $message): void
{
    if (!$condition) {
...
phase6_expect(predictions_data_directory() === $root . '/private/data', 'DASHBOARD_DATA_DIR was not honoured.');
phase6_expect(predictions_database_path() === $root . '/private/database/predictions.sqlite', 'PREDICTIONS_DB_PATH was not honoured.');
...
phase6_expect(is_file($databasePath), 'Predictions SQLite database was not created.');
phase6_expect((fileperms($databasePath) & 0777) === 0600, 'Predictions SQLite database is not mode 0600.');
...
foreach (['index.php', 'league.php', 'card.php', 'history.php', 'leaderboard.php', 'results.php'] as $page) {
    ...
    phase6_expect($source !== false && str_contains($source, 'href="../"'), $page . ' does not link back to the dashboard.');
}

$badPath = $root . '/missing/predictions.sqlite';
try {
    predictions_database($badPath);
    phase6_expect(false, 'Missing private database directory was silently accepted.');
} catch (RuntimeException $exception) {
    phase6_expect(str_contains($exception->getMessage(), 'directory is not writable'), 'Database path failure was not actionable.');
}

echo "Phase 6 predictions integration tests passed.\n";
